...
struct StereoCameraSubscriber::Impl {
  ...
  void checkImagesSynchronized() {
    if ( all_received_ != image_received_left_ or
         all_received_ != info_received_left_ or
         all_received_ != image_received_right_ or
         all_received_ != info_received_right_ ) {
...
bool StereoCameraSubscriber::report( bool on ) {
  if ( on != impl_->reporting_ ) {
    ...
    if ( on ) {
      ...
      std::cout << "report disconnect 2 " << std::endl;
      impl_->left_info_report_.disconnect();
      std::cout << "report disconnect 3 " << std::endl;
      impl_->right_image_report_.disconnect();
      std::cout << "report disconnect 4 " << std::endl;
      impl_->right_info_report_.disconnect();
    }
    std::cout << "report register done " << std::endl;
  }
